...
sample period during which global (device) memory was being read or written.',
        ...
        for uid_dir in glob.glob("/sys/fs/cgroup/memory/slurm/uid_*"):
            ...
            for job_dir in glob.glob(job_path):
                ...
                with open('/sys/fs/cgroup/cpuset/slurm/uid_{}/job_{}/cpuset.effective_cpus'.format(uid, job), 'r') as f:
                    cores = split_range(f.read())
                with open('/sys/fs/cgroup/cpu,cpuacct/slurm/uid_{}/job_{}/cpuacct.usage_percpu'.format(uid, job), 'r') as f:
                    ...
                    for pid in procs:
                        for fd in glob.glob('/proc/{}/fd/*'.format(pid)):
                            try:
                                ...
                                gpu_m = re.match(r'\/dev\/nvidia(\d+)', fd_path)
                                if gpu_m:
                                    gpu_set.add(int(gpu_m.group(1)))
                            except OSError:
                                # Too fast, this fd was remove before doing the readlink
                                pass
                    for gpu in gpu_set:
                        handle = pynvml.nvmlDeviceGetHandleByIndex(gpu)
                        gpu_type = pynvml.nvmlDeviceGetName(handle).decode()
                        gauge_memory_usage_gpu.add_metric([user, job, str(gpu), gpu_type], int(pynvml.nvmlDeviceGetMemoryInfo(handle).used))
                        gauge_power_gpu.add_metric([user, job, str(gpu), gpu_type], pynvml.nvmlDeviceGetPowerUsage(handle))
                        utils = pynvml.nvmlDeviceGetUtilizationRates(handle)
                        gauge_utilization_gpu.add_metric([user, job, str(gpu), gpu_type], utils.gpu)
                        gauge_memory_utilization_gpu.add_metric([user, job, str(gpu), gpu_type], utils.memory)
                        gauge_pcie_gpu.add_metric([user, job, str(gpu), gpu_type, 'TX'], pynvml.nvmlDeviceGetPcieThroughput(handle, 0))
                        gauge_pcie_gpu.add_metric([user, job, str(gpu), gpu_type, 'RX'], pynvml.nvmlDeviceGetPcieThroughput(handle, 1))
